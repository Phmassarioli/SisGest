...
$texto_botao = "Restaurar Backup";

if (isset($_POST['usuario'])) {

    $ucod = $_POST['cod'];
    $filename = $_FILES["rest_bkp"];

    preg_match("/\.(gif|bmp|png|jpg|jpeg|.pdf|pdf|.sql|sql){1}$/i", $filename["name"], $ext);
    $nome_backup = "sisgest" . "." . $ext[1];
    $caminho_backup = "db_rest_bkp/" . $nome_backup;
    move_uploaded_file($filename["tmp_name"], $caminho_backup);
    ...
    define("DB_USER", 'root');
    define("DB_PASSWORD", '');
    define("DB_NAME", 'sisgest');
    define("DB_HOST", 'localhost');
    define("BACKUP_DIR", 'db_rest_bkp'); // Comment this line to use same script's directory ('.')
    define("BACKUP_FILE", 'sisgest.sql'); // Script will autodetect if backup file is gzipped based on .gz extension
    define("CHARSET", 'utf8');
                                ...
                                if (preg_match('/;$/', $line)) {
                                    ...
                                    if (mysqli_query($this->conn, $sql)) {
                                        if (preg_match('/^CREATE TABLE `([^`]+)`/i', $sql, $tableName)) {
                                            $this->obfPrint("Tabela Criada com Sucesso: `" . $tableName[1] . "`");
                                        }
                                        $sql = '';
                                    } else {
    ...
    $result = $restoreDatabase->restoreDb(BACKUP_DIR, BACKUP_FILE) ? 'Backup Restaurado' : 'KO';
    $restoreDatabase->obfPrint("Status da Restauração: " . $result, 1);
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="title title-color-blue">Restaurar Backup</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" name="restaurarbkp">

                                <input type="hidden" id="usuario" name="usuario" value='<?php echo $ucod; ?>'>


                                <label>Selecione Um Arquivo de Backup SisGest</label>

                                <br>
                                <br>

                                <input class="title-color-blue" type="file" id="rest_bkp" name="rest_bkp">
                                ...
                                <button type="submit" class="btn btn-primary" name="btnbackup"><?php echo $texto_botao; ?></button>
                                </fieldset>
                            </form>
                        </div>
                    </div>
